...
| **Board da Sprint** | Visão admin do kanban com filtro de guilda server-side (`?guild_id=`); estado persiste na sessão; cards compactos com scroll por coluna; modal de detalhes com edição, checklist (inclui adicionar itens inline), comentários e subtasks (XP, Gold, breadcrumb pai, seta ↗ maior); badge `[done/total]` no card; kanban do player exibe apenas quests de sprints ativas; **colunas personalizadas por guilda** (#72/#73) — admin edita via modal ⚙️ ao selecionar uma guilda; **seletor de coluna no modal** (#103) — admin move qualquer quest para qualquer coluna (bypass de in_progress para done); **color picker nas colunas** (#102) — cor persiste no PATCH; stale column_id corrigido (#102) |
